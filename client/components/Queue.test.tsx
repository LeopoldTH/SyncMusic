...
describe("file de lecture", () => {
  it("invite a ajouter quand la file est vide", () => {
    const html = renderToStaticMarkup(<Queue items={[]} currentItemId={null} youAre="leo" onRemove={noop} />);
  ...
  it("n affiche pas de bouton retirer sur le morceau en cours", () => {
    const html = renderToStaticMarkup(<Queue items={items} currentItemId="q1" youAre="leo" onRemove={noop} />);
  ...
  it("affiche un bouton retirer sur chaque morceau quand rien ne joue", () => {
    const html = renderToStaticMarkup(<Queue items={items} currentItemId={null} youAre="leo" onRemove={noop} />);
...
describe("qui a ajoute quoi", () => {
  it("dit toi pour ses propres ajouts et ton pote pour les autres", () => {
    ...
    expect(html).toContain("ajoute par ton pote");
  });
