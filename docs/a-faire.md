...
- **Barre de recherche.** Décidée : endpoint `search` officiel de la YouTube Data API v3, déclenchée à l'Entrée, quota 100 requêtes/jour accepté pour la V1. **Bloquée sur une clé d'API** : le projet n'en a aucune aujourd'hui, `server/videoTitle.ts` passe par oEmbed précisément parce que c'est sans clé et sans quota. Il faut créer une clé dans la console Google Cloud (même projet que les identifiants OAuth), la poser en `YOUTUBE_API_KEY` dans `.env` et en secret Fly. Le code peut s'écrire avant, il ne peut pas se vérifier de bout en bout sans elle.
